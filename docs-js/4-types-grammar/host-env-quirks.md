# IV Types & Grammar (You Don't Know JS)


## Mixed Environment JavaScript
- JS runs in the context of hosting env, so there's some unpredictability
- due to different: authors (dependencies), JS engines, browsers.


## Deviations from the spec and implementation

*ECMAScript spec* Annex B lists deviations from the spec:
- Octal literals allowed in sloppy mode, e.g. `0123`
- `window.escape|unescape` use %-delim hex seq: "?f=9%&" -> "%3Ff%3D9%25%26"
- `String#substr`   : 2nd param is 'length', i.e. num of chars to include
- `String#substring`: 2nd param is noninclusive ending index


*Web ECMAScript spec* covers differences between official spec and current implementations in browsers: http://javascript.spec.whatwg.org/. These items are required of browsers in order to be compatible with each other (but are not listed in Annex B):

* `<!--` and `-->` are valid single-line comment delimiters in JS.

* _String.prototype.*_ methods for returning HTML-formatted strings: 
  `link(url)  anchor(url)   fontcolor(color)  fontsize(n)
   bold italics   strike fixed   sup sub   big small   blink`
  (rarely used in practice, other DOM APIs or user fn favored)

* _RegExp extensions_: `RegExp.$1` .. `RegExp.$9` (match-groups) 
  and `RegExp.lastMatch` / `RegExp["$&"]` (most recent match).

* _Function.prototype_ methods: 
  `Function.prototype.arguments` (aliases internal `arguments` object) 
   and `Function.caller` (aliases internal `arguments.caller`)
   (arguments, arguments.caller, and these aliases are deprecated).


## Host Objects

Var behavior has exceptions for auto-defined vars, i.e. the ones provided by JS hosts like browser or node; these *host objects* include both built-in objects and functions.
