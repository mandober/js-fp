...
a = null;       typeof a;       // "object" (BUG)
a = undefined;  typeof a;       // "undefined"
a = { b: "c" }; typeof a;       // "object"
```

- The return value from the `typeof` operator is always one of 7 string values. 
- Only values have types; variables are just simple containers for those values.
- `typeof null` is an interesting case, because it errantly returns `"object"`, when you'd expect it to return `"null"` (long-standing bug in JS).
- setting `a = undefined` is no different then unint variable 
...
- `object` type is a compound value with properties that hold a value.
...
Props can be accessed with dot or bracket notation
...
The bracket notation requires either a var or a string literal.

bracket notation is also useful if you want to access a property/key but the name is stored in another variable

*array* and *function*: rather than being proper built-in types, these should be thought of more like subtypes - specialized versions of the `object` type.
...
functions are a subtype of `objects`, which implies that a `function` is a main type - and can thus have properties, but you typically will only use function object properties (like `foo.bar`) in limited cases.
