...
## Statements
- JS programs are collections of statements
- statements perform an action; some even return a value (very expr-like)
  e.g. `a = 2` returns 2, so one can do `b = a = 2` 
- above, 2 is a literal value, = is an operator (form of a fn)
- statements are made up of one or more expressions


## Expressions
- expr is a ref to a var/val, or var/val set combined with operators
  `a = b * 2;` has 4 expressions:
  - *literal value expression*, `2`
  - *variable expression*, `b`
  - *arithmetic expression*, `b * 2`
  - *assignment expression*, `a = b * 2`
- a general standalone expr is *expression statement*, e,g, `b * 2`
- common expression statement is *call expression* statement as the entire
  statement is the function call expression itself: `alert( a )`


## Program Execution
- JS is interpreted, but actually JIT compiled
...
More: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Expressions_and_Operators


## Values & Types

- JS has built-in types i.e. *primitive* value
 `string`, `number`, `boolean` value types, it also has:
